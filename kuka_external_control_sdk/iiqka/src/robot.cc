      ...
      .SetServerErrorCallback(
        [this](os::core::udp::communication::SecureReplier & replier)
        {
          ...
          event_handler_->OnError(
            "Connecting to server failed with error code: " + std::to_string(error_ret_pair.first) +
...
Status Robot::StartControlling(ControlMode control_mode)
{
  ...
  {
    return Status(
      ReturnCode::ERROR, "StartControlling failed: network connection not initialized.");
  }
  ...
  return event_handler_set_
           ...
           : Status(
               ReturnCode::WARN, "Warning: StartControlling called with default event handler.");
}
...
Status Robot::StopControlling()
{
  ...
  if (!replier_socket_->IsRequestActive())
  {
    ...
    if (
      this->ReceiveMotionState(std::chrono::milliseconds(kStopRecvTimeout)).return_code !=
...
Status Robot::CreateControllingSubscription()
{
  ...
  {
    return Status(
      ReturnCode::ERROR, "ObserveControlState failed: network connection not initialized.");
  }
...
Status Robot::CancelControllingSubscription()
{
  ...
  {
    return Status(
      ReturnCode::ERROR, "StopObservingControlState failed: observing is not in progress.");
  }
  ...
  std::function<void(BaseMotionState &)> handle_motion_states_)
{
  ...
  monitoring_thread_ = std::thread(
    [=]()
    {
      ...
      {
        if (
          subscriber_socket_->WaitForAndReceive(
...
Status Robot::SendControlSignal()
{
  ...
  {
    return Status(
      ReturnCode::ERROR, "SendControlSignal failed: network connection not initialized.");
  }
  ...
  {
    return Status(
      ReturnCode::ERROR, "SendControlSignal failed: please fill out the control signal first.");
  }
...
Status Robot::ReceiveMotionState(std::chrono::milliseconds receive_request_timeout)
{
  ...
  {
    return Status(
      ReturnCode::ERROR, "ReceiveMotionState failed: network connection not initialized.");
  }
...
Status Robot::RegisterEventHandler(std::unique_ptr<EventHandler> && event_handler)
{
  ...
  {
    return Status(
      ReturnCode::ERROR, "RegisterEventHandler failed: please provide a valid pointer.");
  }
