...
Status Robot::Setup() {
  Reset();
  auto replier_address =
      os::core::udp::communication::SocketAddress::SafeConstruct(
          config_.client_ip_address.c_str(), config_.udp_replier_port);

  auto subscriber_address =
      os::core::udp::communication::SocketAddress::SafeConstruct(
          config_.udp_subscriber_multicast_address.c_str(),
          config_.udp_subscriber_port);

  auto interface_address =
      os::core::udp::communication::SocketAddress::SafeConstruct(
          config_.client_ip_address);
  ...
  if (config_.is_secure) {
    replier_socket_ =
        std::make_unique<os::core::udp::communication::SecureReplier>(
            config_.certificate_path, config_.private_key_path,
            replier_address.value());
    dynamic_cast<os::core::udp::communication::SecureReplier &>(
        *replier_socket_)
        .SetServerErrorCallback(
            [this](os::core::udp::communication::SecureReplier &replier) {
              std::lock_guard<std::mutex> lck(event_handler_mutex_);
              const auto &error_ret_pair =
                  replier.Socket().GetLastSocketError();
              event_handler_->OnError(
                  "Connecting to server failed with error code: " +
                  std::to_string(error_ret_pair.first) +
                  " and message: " + std::to_string(error_ret_pair.second));
              return true;
            });
  } else {
    replier_socket_ = std::make_unique<os::core::udp::communication::Replier>(
        replier_address.value());
  }

  subscriber_socket_ =
      std::make_unique<os::core::udp::communication::Subscriber>(
...
Status Robot::SetQoSProfile(QoS_Configuration qos_config) {
  ...
  request.mutable_qos_profiles(0)
      ->mutable_rt_packet_loss_profile()
      ->set_timeframe_ms(qos_config.timeframe_ms);
...
Status Robot::StartControlling(ControlMode control_mode) {
  ...
  return event_handler_set_
             ...
             : Status(ReturnCode::WARN, "Warning: StartControlling called with "
...
Status Robot::StopControlling() {
  ...
  if (!replier_socket_->IsRequestActive()) {
    // Wait for next motion state, if no request is active to be able to send
    // valid stop signal Timeout is default 6 ms
    if (this->ReceiveMotionState(std::chrono::milliseconds(kStopRecvTimeout))
            .return_code != kuka::external::control::ReturnCode::OK)
      return Status(ReturnCode::ERROR,
  ...
  monitoring_thread_ = std::thread([&]() {
    ...
    while (!stop_monitoring_) {
      if (subscriber_socket_->WaitForAndReceive(
        ...
        if (monitoring_arena.ParseFromArray(received_msg.first,
                                            received_msg.second)) {
...
Status Robot::SendControlSignal() {
  ...
  if (!controlling_arena_.SerializeExistingMessageToArray(encode_buffer,
                                                          size)) {
    return Status(ReturnCode::ERROR,
...
Robot::ReceiveMotionState(std::chrono::milliseconds receive_request_timeout) {
  ...
  if (recv_ret == os::core::udp::communication::Socket::ErrorCode::kSuccess) {
    ...
    if (!monitoring_arena_.ParseFromArray(req_message.first,
                                          req_message.second)) {
...
bool Robot::HasMonitoringSubscription() {
  return monitoring_thread_.joinable();
}
...
bool Robot::Uninitialized() {
  return stub_ == nullptr || replier_socket_ == nullptr ||
         subscriber_socket_ == nullptr;
}
...
Status
Robot::ConvertStatus(os::core::udp::communication::Socket::ErrorCode code) {
...
void Robot::SetupGRPCChannel() {
  stub_ = kuka::ecs::v1::ExternalControlService::NewStub(grpc::CreateChannel(
      config_.koni_ip_address + ":" + std::to_string(config_.ecs_grpc_port),
