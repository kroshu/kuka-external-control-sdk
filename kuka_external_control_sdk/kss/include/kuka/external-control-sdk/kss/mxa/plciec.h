...
#define MIN(a, b) (((a) < (b)) ? (a) : (b))
  ...
  template <typename T>
  inline bool IsBitSet(T value, std::size_t idx)
  {
    using U = std::make_unsigned_t<T>;
    return (static_cast<U>(value) & (static_cast<U>(1) << idx)) != 0;
  }

  /* Set bit @ pos if val is true, clear bit @ pos if val is false */
  template <typename T>
  inline void ChBit(T &value, std::size_t idx, bool set)
  {
    using U = std::make_unsigned_t<T>;
    U mask = static_cast<U>(1) << idx;

    if (set)
    {
      value = static_cast<T>(static_cast<U>(value) | mask);
    }
    else
    {
      value = static_cast<T>(static_cast<U>(value) & ~mask);
  ...
  public:
    TON()
        : IN(FALSE), PT(0), Q(_Q), ET(_ET), _Q(FALSE), _ET(0), m_PrevIn(FALSE), m_PrevTime(0), m_Active(FALSE) {}
  ...
  static inline INT DINT_TO_INT(DINT arg) { return static_cast<INT>(arg); }
  static inline DWORD DINT_TO_DWORD(DINT arg) { return *reinterpret_cast<DWORD *>(&arg); }
  static inline TIME DINT_TO_TIME(DINT arg) { return static_cast<DINT>(arg); }
  static inline DINT DWORD_TO_DINT(DWORD arg) { return *reinterpret_cast<DINT *>(&arg); }
  static inline DINT INT_TO_DINT(INT arg) { return static_cast<DINT>(arg); }
  static inline BYTE INT_TO_BYTE(INT arg) { return static_cast<BYTE>(arg); }
  static inline INT WORD_TO_INT(WORD arg) { return static_cast<INT>(arg); }
  static inline DINT REAL_TO_DINT(REAL arg) { return static_cast<DINT>(floor(arg + 0.5f)); }
  static inline INT REAL_TO_INT(REAL arg) { return static_cast<INT>(floor(arg + 0.5f)); }
  static inline DWORD REAL_TO_DWORD(REAL arg) { return DINT_TO_DWORD(static_cast<DINT>(floor(arg + 0.5f))); }
  static inline BOOL REAL_TO_BOOL(REAL arg) { return arg ? TRUE : FALSE; }
  static inline INT BYTE_TO_INT(BYTE arg) { return static_cast<INT>(arg); }
  static inline SINT BYTE_TO_SINT(BYTE arg) { return static_cast<SINT>(arg); }
}
