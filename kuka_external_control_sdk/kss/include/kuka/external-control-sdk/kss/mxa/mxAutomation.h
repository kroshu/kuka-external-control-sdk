...
public:
   ...
   // Only available from mxA 4.0 upwards
...
public:
   void OnCycle()
   {
      ...
      if (MXA_EXECUTECOMMAND_1.READCMDDATARET)
      {
         ...
         // Number of axes and dynamics only supported from mxA 4.0 upwards
         if (_KRC_MAJOR > 3)
         {
         ...
         else
         {
            _KRC_NUM_ROB_AXIS = 0;
            _KRC_NUM_EX_AXIS = 0;
            KRC_AXISGROUPREFARR[AXISGROUPIDX].VEL_MAX_CP = 0;
            KRC_AXISGROUPREFARR[AXISGROUPIDX].ACC_MAX_CP = 0;
         }
