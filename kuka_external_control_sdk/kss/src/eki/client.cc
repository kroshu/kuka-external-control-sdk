...
Status Client::Start()
{
  ...
  return connect_status.return_code == ReturnCode::OK
           ? (event_handler_set_
                ...
                : Status(
                    ReturnCode::WARN, "EKI communication started but default event handler used"))
           : connect_status;
}
...
Status Client::SendMessageAndWait()
{
  ...
  bool req_success = req_response_cv_.wait_for(
    recv_lk, std::chrono::seconds(4), [this] { return !request_active_; });
...
Status Client::RegisterEventHandlerExtension(std::unique_ptr<IEventHandlerExtension> && extension)
{
  ...
  {
    return {
      ReturnCode::ERROR, "RegisterEventHandlerExtension failed: please provide a valid pointer"};
  }
...
Status Client::RegisterStatusResponseHandler(std::unique_ptr<IStatusUpdateHandler> && handler)
{
  ...
  {
    return {
      ReturnCode::ERROR, "RegisterStatusResponseHandler failed: please provide a valid pointer"};
  }
...
Status Client::TurnOnDrives() { return SendCommand(CommandType::DRIVES_ON); }

Status Client::TurnOffDrives() { return SendCommand(CommandType::DRIVES_OFF); }

Status Client::SetCycleTime(CycleTime cycle_time) { return SendCycleTimeChange(cycle_time); }
