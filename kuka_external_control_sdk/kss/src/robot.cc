  ...
  switch (config.installed_interface)
  {
    ...
    default:
      throw std::runtime_error(
        "Configuration contains invalid interface, please choose between EKI, MXA or plain RSI.");
  };
}

Status Robot::Setup() { return installed_interface_->Setup(); }
...
Status Robot::StartMonitoring() { return installed_interface_->StartMonitoring(); }

Status Robot::StopControlling() { return installed_interface_->StopControlling(); }

Status Robot::StopMonitoring() { return installed_interface_->StopMonitoring(); }
...
Status Robot::SendControlSignal() { return installed_interface_->SendControlSignal(); }
