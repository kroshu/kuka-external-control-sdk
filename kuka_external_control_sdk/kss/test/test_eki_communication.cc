...
TEST(EkiCommunication, ParseInitializationDataAndSucceed) {
  ...
  for (size_t i = 0; i < init_data.num_axes; ++i)
    ASSERT_EQ(init_data.axis_type[i], "ROTATIONAL");

  for (size_t i = init_data.num_axes; i < init_data.axis_type.size(); ++i)
    ASSERT_EQ(init_data.axis_type[i], "LINEAR");
