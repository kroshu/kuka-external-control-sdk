...
TEST_F(iiQKARobot, CheckControllingAddVelocity) {
  kuka::external::control::ReturnCode setup_ret = robot_->Setup().return_code;
  EXPECT_EQ(setup_ret, kuka::external::control::ReturnCode::OK);
  ...
  for (int i = 0; i < 15; ++i) {
    if (i % 2 == 0) {
      if (i == 0) {
        EXPECT_EQ(robot_->GetLastMotionState().GetMeasuredVelocities(), nullptr);
      } else {
        for (int i = 0; i < 6; ++i) {
          EXPECT_NEAR(robot_->GetLastMotionState().GetMeasuredVelocities()->at(i), 0.0, 0.01);
        }
      }
      send_velocities = std::vector<double>{7.0, 7.0, 7.0, 7.0, 7.0, 7.0, 7.0, 7.0};
      robot_->GetControlSignal().AddJointPositionValues(send_velocities);
      send_velocities = std::vector<double>{4.0, 4.0, 4.0};
      robot_->GetControlSignal().AddJointPositionValues(send_velocities);
    } else {
      for (int i = 0; i < 6; ++i) {
        EXPECT_NEAR(robot_->GetLastMotionState().GetMeasuredVelocities()->at(i), 0.0, 0.01);
      }
      send_velocities.resize(0);
      robot_->GetControlSignal().AddJointPositionValues(send_velocities);
    }
...
TEST_F(iiQKARobot, CheckControllingAddPositions) {
  kuka::external::control::ReturnCode setup_ret = robot_->Setup().return_code;
  EXPECT_EQ(setup_ret, kuka::external::control::ReturnCode::OK);
  ...
  for (int i = 0; i < 15; ++i) {
    if (i % 2 == 0) {
      if (i == 0) {
        EXPECT_EQ(robot_->GetLastMotionState().GetMeasuredPositions(), nullptr);

        EXPECT_EQ(robot_->ReceiveMotionState(std::chrono::seconds(8)).return_code,
                  kuka::external::control::ReturnCode::OK);
      } else {
        EXPECT_EQ(*(robot_->GetLastMotionState().GetMeasuredPositions()),
                  std::vector<double>({4.0, 4.0, 4.0, 7.0, 7.0, 7.0}));
      }
      send_positions = std::vector<double>{7.0, 7.0, 7.0, 7.0, 7.0, 7.0, 7.0, 7.0};
      robot_->GetControlSignal().AddJointPositionValues(send_positions);
      send_positions = std::vector<double>{4.0, 4.0, 4.0};
      robot_->GetControlSignal().AddJointPositionValues(send_positions);

    } else {
      EXPECT_EQ(*(robot_->GetLastMotionState().GetMeasuredPositions()),
                std::vector<double>({4.0, 4.0, 4.0, 7.0, 7.0, 7.0}));
      send_positions.resize(0);
      robot_->GetControlSignal().AddJointPositionValues(send_positions);
    }
